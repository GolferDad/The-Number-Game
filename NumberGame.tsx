...
import { Trophy, RotateCcw, Sparkles, Settings, Zap } from 'lucide-react';
import './number-game.css';

const NumberGame = () => {
  const [gameState, setGameState] = useState('menu'); // menu, setup, manualEntry, playing, powerup, arrangement, gameOver
  const [gameMode, setGameMode] = useState('auto'); // auto or manual
  const [partyMode, setPartyMode] = useState(false);
  const [playerCount, setPlayerCount] = useState(2);
  const [playerNames, setPlayerNames] = useState(['', '', '', '']);
  const [soloStats, setSoloStats] = useState({ gamesWon: 0, bestRounds: null, currentStreak: 0, bestStreak: 0 });
  ...
  const [bestGuessCount, setBestGuessCount] = useState(0);
  const [currentGuess, setCurrentGuess] = useState(['', '', '']);
  const [guessesRemaining, setGuessesRemaining] = useState(5);
  const [arrangementsRemaining, setArrangementsRemaining] = useState(2);
  const [foundDigits, setFoundDigits] = useState([]);
  ...
  const [revealedDigit, setRevealedDigit] = useState(null);
  ...
  const powerUps = [
    ...
    { id: 'skip-turn', name: 'Skip Turn!', icon: '⏭️', description: 'Miss this turn entirely' },
  ...
  const spinPowerUpWheel = () => {
    ...
    setTimeout(() => {
      ...
      } else if (randomPowerUp.id === 'chug-2' || randomPowerUp.id === 'chug-5' || 
                 randomPowerUp.id === 'finish-drink' || randomPowerUp.id === 'drink-and-guess' ||
                 randomPowerUp.id === 'double-nothing') {
  ...
  const startRound = () => {
    ...
    // Solo mode always uses auto-generated numbers
  ...
  // Move the setTimeout OUTSIDE the if/else block
  setTimeout(() => {
  ...
  const checkGuess = () => {
    ...
    // Apply wildcard if active (only on first guess)
    let matches = 0;
    guess.forEach(digit => {
    ...
    if (activePowerUp?.id === 'wildcard' && guesses.length === 0) {
      ...
      // If 3 right, wildcard doesn't show a message (they found all digits anyway)
      if (matches === 3) {
        setActivePowerUp(null);
      }
    }
    ...
    // Show immediate feedback (especially important when history is hidden)
    if (matches < 3) {
    ...
    // Track best guess count for consolation scoring
    ...
    if (matches === 3 && gameState === 'playing') {
      ...
      // Consolation scoring based on best guess
  ...
  const handleInputChange = (index, value, isManual = false) => {
  ...
  const handleKeyDown = (index, e, isManual = false) => {
